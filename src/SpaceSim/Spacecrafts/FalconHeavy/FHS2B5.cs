...
namespace SpaceSim.Spacecrafts.FalconHeavy
{
    sealed class FHS2B5 : SpaceCraftBase
    {
        ...
        public FHS2B5(string craftDirectory, DVector2 position, DVector2 velocity, double zOffset = 9)
            : base(craftDirectory, position, velocity, 0, 105750, "Falcon/9/S2.png")
        {
            StageOffset = new DVector2(0, zOffset);

            Engines = new IEngine[]
            {
                new Merlin1DB5Vac(this, new DVector2(0, Height * 0.38))
            };
        }
    }
}

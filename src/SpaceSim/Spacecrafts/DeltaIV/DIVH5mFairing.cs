...
{
    class DIVH5mFairing : SpaceCraftBase
    {
        ...
        public DIVH5mFairing(string craftDirectory, DVector2 position, DVector2 velocity, bool isLeft)
            : base(craftDirectory, position, velocity, 0, 0, isLeft ? "SLS/fairingLeft.png" : "SLS/fairingRight.png", null)
        {
            ...
            if (_isLeft)
            {
                StageOffset = new DVector2(-1.24, -4.8);
            }
            else
            {
                StageOffset = new DVector2(1.24, -4.8);
